...
if(isset($_POST['ban'])){
	$stmt2 = $conn->prepare("update users set is_banned = true where id_user = ?") ;
	$h = $_POST['h'] ;
...
<?php
	...
	$stmt = $conn->query("SELECT * FROM users where is_activated = true and is_admin = false and is_banned = false");
