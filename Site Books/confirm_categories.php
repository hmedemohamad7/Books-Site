...
if(isset($_POST['confirm'])){
	$stmt3 = $conn->prepare("update categories set EEE = 1 where name_category = ?") ;
	$nc = $_POST['h'] ;
	$stmt3->bind_param("s",$nc);
	$stmt3->execute();
	
	header("Location: confirm_categories.php");
}

if(isset($_POST['delete'])){
	$stmt2 = $conn->prepare("delete from categories where name_category = ?") ;
	$nc = $_POST['h'] ;
...
<?php
	...
	$stmt = $conn->query("SELECT * FROM categories where EEE != 1");
...
?>
		...
		<div class="w3-twothird w3-container">
		  ...
		  <p>
		     category name: <?php echo $row['name_category'];?> <br/>
		     id user: <?php echo $row['EEE']; ?><br/>
			 <?php $i=$row['name_category']; ?>
	     </p>
		</div>
